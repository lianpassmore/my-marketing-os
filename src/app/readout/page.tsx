...
import { BarChart3, Mail, MousePointerClick, ShieldCheck, AlertTriangle, Loader2, TrendingUp } from 'lucide-react';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (statsRes.ok) {
          const { totals: t, broadcastStats: bs, dailyOpens: do_ } = await statsRes.json();
          setTotals(t);
  ...
  return (
    <main className="p-8 max-w-6xl mx-auto w-full">
      ...
      {isLoading ? (
        ...
        <div className="bg-surface-paper border border-surface-mist rounded-xl shadow-sm flex flex-col items-center justify-center p-12 text-center relative overflow-hidden">
          <div className="absolute inset-0 opacity-5 flex items-end justify-center pointer-events-none">
            <div className="w-full h-1/2 bg-gradient-to-t from-brand-storm to-transparent"></div>
          </div>
          <div className="w-16 h-16 bg-surface-cloud rounded-full flex items-center justify-center mb-6 shadow-inner relative z-10">
            <BarChart3 className="text-brand-storm" size={32} />
          </div>
          <h2 className="text-xl font-semibold text-content-ink mb-2 relative z-10">Awaiting Data</h2>
          <p className="text-content-slate max-w-md mx-auto relative z-10">
